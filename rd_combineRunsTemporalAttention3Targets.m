function rd_combineRunsTemporalAttention3Targets(subject)
...
%     subject = 'rd_cbD10_tilt2_tc16-64_soa1000-1250';
    subject = 'rdPilot_vhD15_tilt2_tc100_soa1000-1250_T3';
end
runs = 1:6;
...
for iRun = 1:nRuns
    run = runs(iRun);
    dataFile = dir(sprintf('%s/%s_run%02d*TemporalAttention3Targets*', dataDir, subject, run));
...
if saveData
    fileName = sprintf('%s/%s_TemporalAttention3Targets_T1T2all_%s.mat', dataDir, subjectID, datestr(now, 'yyyymmdd'));
...
if saveFigs
    figNames = {'acc','rt'};
    rd_saveAllFigs([], figNames, [subjectID '_TemporalAttention3Targets_T1T2all'], figDir)
end
