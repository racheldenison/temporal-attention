function results = rd_temporalAttentionExo(subjectID)
...
while trialCounter <= nTrials
    ...
    % Select tones
    % the cue tone is only a preparatory cue
    ...
    if p.eyeTracking
        while GetSecs < timeCue + p.soas(1) - p.exoSOA - p.eyeSlack && ~stopThisTrial
            WaitSecs(.01);
    ...
    % Check for eye movements
    if p.eyeTracking
        while GetSecs < timeCue + p.soas(2) - p.exoSOA - p.eyeSlack && ~stopThisTrial
            WaitSecs(.01);
%             fixation = mod(iTrial,10); %%% for testing
            fixation = rd_eyeLink('fixcheck', window, {cx, cy, rad});
            [stopThisTrial trialOrder, nTrials] = fixationBreakTasks(...
...
%     if response==respTargetState;
    ...
    % Store trial info
    trials(trialIdx,cuedIntervalIdx) = sum(cuedInterval); % 0=none, 1=T1, 2=T2, 3=both
    ...
    timing.timeExo1(trialIdx,1) = timeExo1;
    timing.timeExoBlank1(trialIdx,1) = timeExoBlank1;
    ...
    timing.timeExo2(trialIdx,1) = timeExo2;
    timing.timeExoBlank2(trialIdx,1) = timeExoBlank2;
...
expt.timing.dur.exo1 = expt.timing.timeExoBlank1 - expt.timing.timeExo1;
expt.timing.dur.exo2 = expt.timing.timeExoBlank2 - expt.timing.timeExo2;
expt.timing.dur.exo1Im1SOA = expt.timing.timeIm1 - expt.timing.timeExo1;
expt.timing.dur.exo2Im2SOA = expt.timing.timeIm2 - expt.timing.timeExo2;

%% Analyze and save data
[expt results] = rd_analyzeTemporalAttentionExo(expt, saveData, saveFigs, plotTimingFigs, saveTimingFigs);
...
if p.eyeTracking
    ...
    % rename eye file
    eyeFileFull = sprintf('%s/%s_TemporalAttentionExo_%s.edf', eyeDataDir, subjectID, datestr(now, 'yyyymmdd'));
