function results = rd_temporalAttentionAdjust(subjectID)
...
p = temporalAttentionAdjustParams;
...
% make target with 0 rotation, for adjustment response
t0 = buildColorGrating(pixelsPerDegree, p.imSize, ...
    spatialFrequency, 0, targetPhase, targetContrast, 0, 'bw');
target0 = maskWithGaussian(t0, size(t0,1), targetSize);
...
% texture for adjustment response
tex0 = Screen('MakeTexture', window, target0*white);
...
switch p.rotateTarget
    ...
    case 'random'
        targetRotations = floor(180*rand(nTrials0,2));
    case 'rotT1'
...
[expt results] = rd_analyzeTemporalAttentionAdjust(expt, saveData, saveFigs, plotTimingFigs, saveTimingFigs);
% % save separately in order to include "Adjust" in the file name
% if saveData
%     fileName = sprintf('data/%s_TemporalAttentionAdjust_%s.mat', subjectID, datestr(now, 'yyyymmdd'));
%     save(fileName, 'expt', 'results')
% end
% if saveFigs 
%     if plotTimingFigs && saveTimingFigs
%         figNames = {'acc','rt','timing','timingHist'};
%     else
%         figNames = {'acc','rt'};
%     end
%     rd_saveAllFigs([], figNames, sprintf('%s_TemporalAttentionAdjust', subjectID))
% end
...
if p.eyeTracking
    ...
    % rename eye file
    eyeFileFull = sprintf('%s/%s_TemporalAttentionAdjust_%s.edf', eyeDataDir, subjectID, datestr(now, 'yyyymmdd'));
