function [expt results] = rd_analyzeTemporalAttentionExo(expt, saveData, saveFigs, plotTimingFigs, saveTimingFigs, T1T2Axis, cleanRT)
...
%% Save data
if saveData
    fileName = sprintf('data/%s_TemporalAttentionExo_T1T2%s_%s.mat', subjectID, T1T2Axis, datestr(now, 'yyyymmdd'));
...
if saveFigs
    figNames = {'acc','rt'};
    rd_saveAllFigs(fig, figNames, sprintf('%s_TemporalAttentionExo_T1T2%s', subjectID, T1T2Axis))
end

%% Plot timing
if plotTimingFigs
    tfig(1) = figure('Color','w','Position',[1 1 800 400]);
    hold on
    ...
    plot(6*ones(nTrials,1), timing.dur.exo1,'o')
    plot(7*ones(nTrials,1), timing.dur.exo2,'o')
    plot(8*ones(nTrials,1), timing.dur.exo1Im1SOA,'o')
    plot(9*ones(nTrials,1), timing.dur.exo2Im2SOA,'o')
    set(gca,'XTick',1:9)
    set(gca,'XTickLabel',{'im 1','im 2','im1-im2 SOA','cue-im1 SOA','cue-im2 SOA','exo 1','exo 2','exo1-im1 SOA','exo2-im2 SOA'})
    ylabel('duration')
    
    tfig(2) = figure('Position',[1 1 1200 300]);
    subplot(1,9,1)
    hist(timing.dur.im1)
    ...
    ylabel('number of trials')
    subplot(1,9,6)
    hist(timing.dur.exo1)
    xlabel('exo 1 duration (s)')
    ylabel('number of trials')
    subplot(1,9,7)
    hist(timing.dur.exo2)
    xlabel('exo 2 duration (s)')
    ylabel('number of trials')
    subplot(1,9,8)
    hist(timing.dur.exo1Im1SOA)
    xlabel('exo1-im1 SOA (s)')
    ylabel('number of trials')
    subplot(1,9,9)
    hist(timing.dur.exo2Im2SOA)
    xlabel('exo2-im2 SOA (s)')
    ylabel('number of trials')
...
%     end
    ...
    if saveTimingFigs
        ...
        rd_saveAllFigs(tfig, figNames, sprintf('%s_TemporalAttentionExo', subjectID))
    end
end
